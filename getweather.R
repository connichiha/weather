weathermain <- function()
{
    ...
    library(mapdata)
    library(maptools)
    map<-readShapePoly('mapdata/bou2_4p.shp')  # 加载中国行政区地图数???
    plot(map)
...
getWeather<-function (x)
{
    ...
    visibility<-as.numeric(sapply(ans, xmlGetAttr, "visibility"))                   # 能见???
    ...
    low<-as.numeric(sapply(ans, xmlGetAttr, "low"))                                 # 最高气???
    high<-as.numeric(sapply(ans, xmlGetAttr, "high"))                               # 最低气???
...
getColors2<-function(map,prov,ctype){
    ...
    fc<-function(x){ADCODE99$ADCODE99[which(as.character(x)==as.character(ADCODE99$prov))]}
    code<-sapply(prov,fc)
...
weather_html <- function(data=data,type='high',output=FALSE,path=''){    # 输入HTML的天气图
    ...
    recharts.eMap <- eMap(df, namevar=1, datavar = 2, title=title)      # 数据JSON化处???
    if(output){                                                         # 输出HTML文件
